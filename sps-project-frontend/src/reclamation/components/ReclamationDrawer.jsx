    ...
    <aside className="app-form-drawer reclamation-drawer is-open" aria-label={complaint ? "Modifier la réclamation" : "Ajouter une réclamation"}>
      ...
      <Form onSubmit={submit} noValidate>
        ...
        <section className="reclamation-form-section"><h3>Réclamation</h3>
          <Form.Group><div className="reclamation-label-action"><Form.Label>Type de réclamation *</Form.Label><button type="button" className="app-inline-add" onClick={() => onManage("type")} title="Gérer les types"><FontAwesomeIcon icon={faPlus} /></button></div><Form.Select value={form.reclamation_type_id} onChange={(event) => changeType(event.target.value)} isInvalid={Boolean(errors.reclamation_type_id)}><option value="">Sélectionner</option>{typeOptions.map((row) => <option key={row.id} value={row.id}>{row.nom}</option>)}</Form.Select>{feedback("reclamation_type_id")}{currentType?.configuration_warning && <Form.Text className="text-warning">{currentType.configuration_warning}</Form.Text>}</Form.Group>
          <Form.Group><Form.Label>Description détaillée *</Form.Label><Form.Control as="textarea" rows={4} maxLength={5000} value={form.description} onChange={(event) => setField("description", event.target.value)} isInvalid={Boolean(errors.description)} />{feedback("description")}</Form.Group>
          <Form.Group><div className="reclamation-label-action"><Form.Label>Canal de réception *</Form.Label><button type="button" className="app-inline-add" onClick={() => onManage("canal")} title="Gérer les canaux"><FontAwesomeIcon icon={faPlus} /></button></div><Form.Select value={form.reclamation_canal_id} onChange={(event) => changeChannel(event.target.value)} isInvalid={Boolean(errors.reclamation_canal_id)}><option value="">Sélectionner</option>{channelOptions.map((row) => <option key={row.id} value={row.id}>{row.nom}</option>)}</Form.Select>{feedback("reclamation_canal_id")}</Form.Group>
          {currentChannel?.est_autre && <Form.Group><Form.Label>Préciser le canal *</Form.Label><Form.Control value={form.canal_precision} maxLength={255} onChange={(event) => setField("canal_precision", event.target.value)} isInvalid={Boolean(errors.canal_precision)} />{feedback("canal_precision")}</Form.Group>}
          <div className="reclamation-form-grid two-columns"><Form.Group><Form.Label>Date de réclamation *</Form.Label><Form.Control type="date" max={localDateValue()} value={form.date_reclamation} onChange={(event) => setField("date_reclamation", event.target.value)} isInvalid={Boolean(errors.date_reclamation)} />{feedback("date_reclamation")}</Form.Group><Form.Group><Form.Label>Priorité *</Form.Label><Form.Select value={form.priorite} onChange={(event) => setField("priorite", event.target.value)} isInvalid={Boolean(errors.priorite)}>{(options.priorites || []).map((row) => <option key={row.value} value={row.value}>{row.label}</option>)}</Form.Select>{feedback("priorite")}</Form.Group></div>
        </section>

        <section className="reclamation-form-section"><h3>Affectation</h3><Form.Group><div className="reclamation-label-action"><Form.Label>Département *</Form.Label><button type="button" className="app-inline-add" onClick={() => onManage("departement")} title="Gérer les départements"><FontAwesomeIcon icon={faPlus} /></button></div><Form.Select value={form.departement_id} onChange={(event) => setField("departement_id", event.target.value)} isInvalid={Boolean(errors.departement_id)}><option value="">Sélectionner</option>{departmentOptions.map((row) => <option key={row.id} value={row.id}>{row.nom}</option>)}</Form.Select>{feedback("departement_id")}</Form.Group></section>
