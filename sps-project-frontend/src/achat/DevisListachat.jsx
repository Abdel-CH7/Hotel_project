...
const DevisListachat = () => {
    ...
    const fetchDevis = async () => {
        try {
            ...
            localStorage.setItem('devisesAchat', JSON.stringify(response.data.devises.data));
    ...
    useEffect(() => {
        const devisFromStorage = localStorage.getItem('devisesAchat');
...
console.log('devis',devis)
            ...
            const lignesFactureData = devis.lignedevis.map(
                (ligneDevis) => ({
...
setFresh(true)
        try {
            ...
            const lignesbonLivraisonData = DevisData.lignedevis.map((ligneDevis) => ({
            ...
            updateCounts(DevisData, 'bonLivraison');
                    ...
                    if (ligneDevisData.id) {
                        ...
                        await axios.put(
                            `http://localhost:8000/api/ligneDevis/${ligneDevisData.id}`,
                            {
                                ...
                                id_facture: ligneDevisData.id_facture,
    ...
    const validateForm = () => {
        ...
        // Validate validation offer selection
        if (!formData.validation_offer) newErrors.validation_offer = "La validation de l'offre est requise.";
      
        // Validate mode of payment
        if (!formData.modePaiement) newErrors.modePaiement = 'Le mode de paiement est requis.';
    ...
    const handlePDF = (devisId,handelV) => {
     ...
     let separatorY=0
        ...
        doc.setFontSize(14);
        doc.setFont("helvetica", "bold");
        doc.text("N° demande de prix:", 10, boxY + 24);
        ...
        if (selectedDevis.lignedevis) {
            ...
            const headersLigneDevis = ["Produit", "Code produit", "Désignation", "Quantité"];
...
` : '';
    ...
    if (printWindow) {
        ...
        const title = `Demande de prix N° : ${selectedDevis.reference}`;
 ...
 ${companyNameSection}
            ...
            <table>
                <thead>     
                    <tr>
                        <th style="background-color: #adb5bd;">Date</th>
                        <th style="background-color: #adb5bd;">Validation de l'offre</th>
                        <th style="background-color: #adb5bd;">Mode de Paiement</th>
                    </tr>
                </thead>
                <tbody>
                    <tr>
            ...
            <table>
                <thead>
                    <tr>
                        <th style="background-color: #adb5bd;">Produit</th>
                        <th style="background-color: #adb5bd;">Code produit</th>
                        <th style="background-color: #adb5bd;">Désignation</th>
                        <th style="background-color: #adb5bd;">Quantité</th>
                    </tr>
                </thead>
                <tbody>
                    ${selectedDevis.lignedevis.map((lignedevis) => {
                        const produit = produits.find(prod => prod.id === lignedevis.produit_id);
                        return `
                            <tr>
                                <td>${lignedevis.produit_id}</td>
    ...
    const filteredDevises = filtereddevises.filter((devis) => devis.type === 'F');
...
console.log('wdefgrb',open,dynamicStyles)
    return (
        <ThemeProvider theme={createTheme()}>
            <Box sx={{...dynamicStyles }}>
                <Box component="main" sx={{ flexGrow: 1, p: 3, mt: 4 }}>
                    <h2 className="mt-3 titreColore">Liste  demande de prix</h2>
                    <div
                  ...
                  {selectedProductsData.map((productData, index) => (
                    ...
                    <tr key={index}>
                      <td style={{ backgroundColor: 'white', width: '50px' }}>
                        <Select
                         styles={{
                            menuPortal: (base) => ({ ...base, zIndex: 999900 }),  // Augmente le z-index pour être au-dessus de tout
                          }}
  ...
  >
    Annuler
                        ...
                        >
                            <table className="table table-bordered" style={{marginTop:'-0px'}}>
                                <thead
                                ...
                                <tr>
                                    ...
                                    <th   className="tableHead">N° demande de prix</th>
                                            ...
                                            <td>
                                                <div>
                                                    {devis.status === "Valider" && (
                                                        <div>
                                                            <Button className="btn btn-sm " variant="success" onClick={() => handleGenerateFacture(devis)}>
                                                            {freshFact?
                        <CircularProgress size="20px" />:'F'

                    } 
                                                            </Button>
                                                            <Button className="btn btn-sm  " variant="outline-success"style={{
                                                                marginRight:'10px'
                                                            }}  >
                                                            {count.factureCount || 0}
                                                            </Button>
                                                            <Button className="btn btn-sm " variant="dark" onClick={() =>handleGenerateBonLivraison(devis)}>
                                                            {fresh?
            ...
            <Dropdown.Menu>
                ...
                <Dropdown.Item onClick={() => handlePDF(devis.id,false)}>sans Entite</Dropdown.Item>
                                                        ...
                                                        <Dropdown>
            <Dropdown.Toggle className="btn btn m-1"
