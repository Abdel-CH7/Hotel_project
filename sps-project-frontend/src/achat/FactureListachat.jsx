...
const FactureListachat = () => {
    ...
    useEffect(() => {
        ...
        if (facturesFromStorage  && clientFromStorage) {
...
console.log('formfactur',FactureData)
    ...
    const handleEdit = (facture) => {
        ...
        if (formContainerStyle.right === "-100%") {
            ...
            setTableContainerStyle({ width:'60%' });
        } else {
            closeForm();
            setFormData({ // Clear form data
                raison_sociale: '',
...
setSelectedProductsData([])
    ...
    const filteredFactures = filteredfactures.filter((facture) => facture.type === 'F');
    ...
    const validateForm = () => {
        ...
        // Validate N Facture
        if (!formData.reference) newErrors.reference = 'N° de facture est requis.';
        ...
        // Validate REF BL N
        if (!formData.ref_BL) newErrors.ref_BL = 'Le REF BL N est requis.';
      
        // Validate REF BC N
        if (!formData.ref_BC) newErrors.ref_BC = 'Le REF BC N est requis.';
      
        // Validate TOTAL TTC
        if (!formData.total_ttc || isNaN(formData.total_ttc) || formData.total_ttc <= 0)
          newErrors.total_ttc = 'Le total TTC doit être un nombre positif.';
        ...
        <ThemeProvider theme={createTheme()}>
            <Box sx={{...dynamicStyles}}>
                <Box component="main" sx={{ flexGrow: 1, p: 3, }}>
                    <Toolbar />
                    <div className="">
                        <h2 className="titreColore">Liste des Factures </h2>
                    </div>
...
export default FactureListachat;